...
export class RecoveryService {
  ...
  async updateRecovery(userId: string, dto: UpdateRecoveryDto) {
    const current = await this.prisma.userRecovery.findUnique({
      where: { userId },
    });
    ...
    const totalCalculated = parseFloat((streakPart + sleepPart + moodPart).toFixed(2));

    
    const recoveryPercentage = Math.max(current.recoveryPercentage, totalCalculated);
